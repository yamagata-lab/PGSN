...
# ------------------------------------------------------------------ #
# `with_inherit_chain` must reach every nested object
#
# `gsn_tree` classifies a node by walking `__parent_classes__` and matching
# against the GSN types. `to_python` attaches that chain only when
# `with_inherit_chain=True`, and only to the node it is called on unless the
# flag is passed down its recursive calls. Without it every nested support,
# sub-goal and solution loses the chain and renders as a bare record rather
# than as its GSN type, which is what these two guard.
#
# They were written inside the body of another test, where pytest never
# collected them and nothing they assert was ever checked.
# ------------------------------------------------------------------ #
...
def _gsn_objects(data, path="root"):
    """Yield (path, dict) for every dict that is a PGSN object."""
...
def test_root_and_children_resolve_expected_types():
    ...
    def gsn_type_of(d):
        return next((c for c in d.get("__parent_classes__", [])
                     if c in GSN_TYPES), None)
